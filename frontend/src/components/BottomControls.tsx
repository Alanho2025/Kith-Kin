...
export function BottomControls({ onCommand }: BottomControlsProps) {
  return (
    <nav className="grid grid-cols-2 gap-3 border-t border-slate-200 bg-white p-4 sm:grid-cols-4" aria-label="对话控制">
      ...
      <button type="button" className={`${CONTROL_CLASS} bg-red-600 text-white focus-visible:ring-red-300`} onClick={() => onCommand({ eventType: "session.end", payload: { reason: "user_completed" } })}>
        结束
      </button>
    </nav>
  );
