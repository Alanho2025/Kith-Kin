...
export function VisitSummaryPage({ summary, onSend, onSave, onCancel }: VisitSummaryPageProps) {
  const handleSend = () => {
    conversationDebug("summary.send.click", { summary });
    onSend();
  };
  const handleSave = () => {
    conversationDebug("summary.save.click", { summary });
    onSave();
  };
  const handleCancel = () => {
    conversationDebug("summary.cancel.click", { summary });
    onCancel();
  };
    ...
    <main className="min-h-screen bg-cool-canvas p-5 text-navy sm:p-8">
      <section className="mx-auto max-w-2xl bg-white p-6 sm:rounded-3xl sm:border sm:border-slate-200 sm:p-10">
        ...
        <div className="mt-8 grid gap-3">
          <button type="button" onClick={handleSend} className="min-h-12 rounded-xl bg-teal-700 px-5 py-3 text-xl font-bold text-white">发送给家人</button>
          <button type="button" onClick={handleSave} className="min-h-12 rounded-xl border-2 border-slate-300 px-5 py-3 text-xl font-semibold">只保存，不发送</button>
          <button type="button" onClick={handleCancel} className="min-h-12 rounded-xl border-2 border-slate-300 px-5 py-3 text-xl font-semibold text-red-700">取消</button>
        </div>
      </section>
    </main>
  );
